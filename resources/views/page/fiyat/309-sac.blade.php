...
@section('content')
    ...
    <section class="project-details">
        <div class="auto-container">
            <div class="image mb-40"><img src="{{asset('assets/images/resource/309 Paslanmaz Sac Fİyatları.jpeg')}}" alt=""></div>
            <div class="row">
                @include('page.fiyat.sidebar')
                <div class="col-lg-9">
                    <h2>309 Paslanmaz Sac FİYATLARI</h2>
                    ...
                    <br>
                    <br>
                    <h3>309 Paslanmaz Sac FİYAT LİSTESİ</h3>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                        @for($i=0;$i<\App\Models\fiyatListesi::where('type','=',"309_sac")->count();$i++)
                        <tr>
                            <th scope="row">{{$i+1}}</th>
                            <td>{{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['kalite']}}</td>
                            <td>{{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['kalinlik']}}</td>
                            <td>{{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['ebat']}}</td>
                            <td>{{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['kg']}}</td>
                            <td>${{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['birim']}}</td>
                            <td>${{\App\Models\fiyatListesi::where('type','=',"309_sac")->get()[$i]['plaka']}}</td>
                        </tr>
                        @endfor
                        </tbody>
                    </table>
